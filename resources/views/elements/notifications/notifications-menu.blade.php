<nav class="nav nav-pills noti {{$variant == 'desktop' ? 'notifications-nav' : 'notifications-nav-mobile'}}">
    <a class="nav-item nav-link nnot {{!$activeType ? 'active' : ''}}" href="{{route('my.notifications')}}">
    ...
    @foreach($notificationTypes as $type)
        <a class="nav-item nav-link nnot {{$activeType == $type ? 'active' : ''}}" href="{{route('my.notifications',['type' => $type])}}">
            <div class="d-flex justify-content-center">
                @switch($type)
                    @case('messages')
