<form method="POST" action="{{ route('register') }}" id="register-form">
    ...
    <div class="form-group ">
        <div class="">
            <input id="password" type="password" class="form-control @error('password') is-invalid @enderror" required name="password" autocomplete="new-password" placeholder="{{ __('Password') }}" >
            <label class="showpass"><input type="checkbox" onclick="showPass()"><small></small></label>
            @error('password')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
    ...
    <div class="form-group ">
        <div class="">
            <input id="password-confirm" type="password" class="form-control @error('password_confirmation') is-invalid @enderror" required name="password_confirmation" autocomplete="new-password" placeholder="{{ __('Confirm Password') }}">
            <label class="showpass"><input type="checkbox" onclick="showPass2()"><small></small></label>
    ...
    <div class="form-group">
        <div class="custom-control custom-checkbox npl">
            <div class="agree-term">
                ...
                <label class="custom-control-label" for="tosAgree">
                    <span>{{ __('I agree to the') }} <a href="{{route('pages.get',['slug'=>GenericHelper::getTOSPage()->slug])}}">{{ __('Terms & Conditions') }}</a> {{ __('and') }} <a href="{{route('pages.get',['slug'=>GenericHelper::getPrivacyPage()->slug])}}">{{ __('Privacy Policy') }}</a> <br>{{ __('of FANCCI and I confirm that I am at least 18 years old') }}.</span>
                </label>
            </div>
        </div>
    </div>
